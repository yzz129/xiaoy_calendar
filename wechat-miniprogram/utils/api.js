...
function putFile(path, filePath, contentType = 'image/jpeg', options = {}) {
  const token = wx.getStorageSync(TOKEN_KEY)
  return new Promise((resolve, reject) => {
    wx.getFileSystemManager().readFile({
      ...
      success(file) {
        wx.request({
          ...
          method: 'PUT',
          data: file.data,
          header: { Accept: 'application/json', Authorization: `Bearer ${token}`, 'Content-Type': contentType },
          timeout: Number(options.timeout || 60000),
          success(response) {
            if (response.statusCode >= 200 && response.statusCode < 300) { resolve(response.data || {}); return }
            const error = new Error(response.data?.error || `上传失败（${response.statusCode}）`)
            error.statusCode = response.statusCode
            reject(error)
          },
          fail: (result) => reject(new Error(networkErrorMessage(result.errMsg))),
        })
      },
      fail: (result) => reject(new Error(result.errMsg || '无法读取图片')),
    })
  })
}
...
function download(path) {
  ...
  return new Promise((resolve, reject) => {
    wx.downloadFile({
      ...
      timeout: 30000,
      success(result) {
        if (result.statusCode >= 200 && result.statusCode < 300) { resolve(result.tempFilePath); return }
        const error = new Error(`皮肤下载失败（${result.statusCode}）`)
...
module.exports = { API_ORIGIN, request, putFile, download, stream, networkErrorMessage }
