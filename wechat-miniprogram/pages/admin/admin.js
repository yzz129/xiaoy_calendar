...
Page({
  ...
  async openDetail(event) {
    ...
    try {
      ...
      let originalPath = ''
      if (payload.themeOriginal?.available) {
        try { originalPath = await api.download(payload.themeOriginal.url) } catch { /* 后台仍展示原图元数据 */ }
      }
      const detail = {
        ...payload,
        ...
        themeOriginal: payload.themeOriginal?.available ? {
          ...payload.themeOriginal,
          localPath: originalPath,
          updatedLabel: localTime(payload.themeOriginal.updatedAt),
          sizeLabel: formatBytes(payload.themeOriginal.byteSize),
        } : { available: false },
      }
  ...
  previewWallpaper() {
    const path = this.data.detail?.themeOriginal?.localPath
