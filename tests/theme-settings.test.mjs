...
test('custom skin keeps fixed overlays out of the document flow on mobile', () => {
  ...
  assert.match(css, /\.app\.has-custom-skin\s*>\s*\.app-frame\s*\{\s*position:\s*relative;\s*z-index:\s*2;/)
  ...
  assert.match(css, /\.app\.has-custom-skin::before\s*\{\s*content:\s*none;/s)
  assert.match(css, /\.app\.has-custom-skin::after\s*\{[^}]*background-position:\s*center,\s*var\(--skin-position,[^)]+\);[^}]*background-size:\s*cover, cover;/s)
  assert.match(css, /@media\s*\(min-width:\s*3200px\)\s*and\s*\(min-height:\s*1200px\)[^{]*\{[^}]*\.app-frame\s*\{[^}]*width:\s*min\(2100px,\s*60vw\);[^}]*min-height:\s*0;[^}]*zoom:\s*1\.45;/s)
  assert.match(css, /@media\s*\(min-width:\s*3200px\)\s*and\s*\(min-height:\s*1500px\)[^{]*\{[^}]*\.app-frame\s*\{[^}]*width:\s*min\(1950px,\s*52vw\);[^}]*zoom:\s*1\.75;/s)
  assert.match(css, /@media\s*\(min-width:\s*3200px\)\s*and\s*\(min-height:\s*1700px\)[^{]*\{[^}]*\.app-frame\s*\{[^}]*width:\s*min\(1950px,\s*45vw\);[^}]*zoom:\s*2;/s)
  assert.doesNotMatch(css, /\.calendar-shell\s*\{\s*display:\s*grid;\s*grid-template-rows:\s*54px\s+minmax\(540px,\s*1fr\);/s)
